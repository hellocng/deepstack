...
export async function POST(
  ...
  try {
    ...
    // Move the waitlist entry to the top
    WaitlistPositionManager.setSupabaseClient(supabase)
    const success = await WaitlistPositionManager.moveToTop(id)
    ...
    if (!success) {
      ...
      return NextResponse.json(
        { error: 'Failed to move entry to top' },
        { status: 400 }
      )
    }
    ...
    console.error('Error moving waitlist entry to top:', error)
    return NextResponse.json(
