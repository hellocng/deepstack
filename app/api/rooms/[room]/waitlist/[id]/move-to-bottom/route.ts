...
export async function POST(
  ...
  try {
    ...
    // Move the waitlist entry to the bottom
    WaitlistPositionManager.setSupabaseClient(supabase)
    const success = await WaitlistPositionManager.moveToBottom(id)
    ...
    if (!success) {
      ...
      return NextResponse.json(
        { error: 'Failed to move entry to bottom' },
        { status: 400 }
      )
    }
    ...
    console.error('Error moving waitlist entry to bottom:', error)
    return NextResponse.json(
